...
Merging to `main` computes the next version from the latest tag and pushes `vX.Y.Z`; the tag opens a pre-release; promoting it publishes the release. Releases are immutable and nothing is retagged: repositories pin an exact version and Dependabot opens the pull request when a newer one exists.
